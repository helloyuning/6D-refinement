...
def load_sixd(base_path, seq, nr_frames=0, load_mesh=True, subset_models=[]):
    ...
    path = base_path + '\\train_pbr\\{:06d}\\'.format(int(seq))  # corrected
    ...
    for i in range(nr_frames):
        ...
        nr_string = '{:06d}'.format(i) if 'tudlight' in base_path else '{:06d}'.format(i)#corrected
        fr.color = cv2.imread(os.path.join(path, "rgb", nr_string + ".jpg")).astype(np.float32) / 255.0
