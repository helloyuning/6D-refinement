...
def distance_transform(depth):
    ...
    #print(dt_unsigned)
    #cv2.imshow('show_dt', dt_unsigned)
...
def add(poseA, poseB, model):#简单的手动减法计算两个矩阵的距离
    v_A = transform_points(model.vertices, poseA)
    v_B = transform_points(model.vertices, poseB)

    return np.mean(np.linalg.norm(v_A - v_B, axis=1))#求两个矩阵差后，求所有范数列向量的最大值的平均值


def adi(poseA, poseB, model):#自带函数计算两个pose的距离
    v_A = transform_points(model.vertices, poseA)
    v_B = transform_points(model.vertices, poseB)
    dist = cdist(v_A, v_B)
